...
def score_singlechain_backbone(model, alphabet, args):
    ...
    coords, native_seq = fairesm.inverse_folding.util.load_coords(args.pdbfile, args.chain)
    print('Native sequence loaded from structure file:')
    print(native_seq)
    print('\n')

    ll, _ = fairesm.inverse_folding.util.score_sequence(
...
def score_multichain_backbone(model, alphabet, args):
    if torch.cuda.is_available() and not args.nogpu:
        model = model.cuda()
        print("Transferred model to GPU")
    structure = fairesm.inverse_folding.util.load_structure(args.pdbfile)
    ...
    ll, _ = fairesm.inverse_folding.multichain_util.score_sequence_in_complex(
