...
model, alphabet = fairesm.pretrained.esm_if1_gvp4_t16_142M_UR50()
model = model.eval()
```
...
```
import fairesm.inverse_folding
structure = fairesm.inverse_folding.util.load_structure(fpath, chain_id)
coords, seq = fairesm.inverse_folding.util.extract_coords_from_structure(structure)
```
...
```
structure = fairesm.inverse_folding.util.load_structure(fpath, chain_ids)
coords, native_seqs = fairesm.inverse_folding.multichain_util.extract_coords_from_complex(structure)
```
...
```
import fairesm.inverse_folding
sampled_seq = fairesm.inverse_folding.multichain_util.sample_sequence_in_complex(
...
```
ll_fullseq, ll_withcoord = fairesm.inverse_folding.util.score_sequence(model, alphabet, coords, seq)
```
...
```
ll_fullseq, ll_withcoord = fairesm.inverse_folding.multichain_util.score_sequence_in_complex(
...
```
rep = fairesm.inverse_folding.util.get_encoder_output(model, alphabet, coords)
```
...
```
rep = fairesm.inverse_folding.multichain_util.get_encoder_output_for_complex(
